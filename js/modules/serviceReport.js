...
const ServiceReportModule = {
    ...
    exportWord() {
        ...
        const d = this.getFormData();
        const sections = [
            { heading: '一、问题描述', content: d.problem },
            { heading: '二、处理过程', content: d.process },
            { heading: '三、处理结果', content: d.result }
        ];
        if (d.notes) sections.push({ heading: '四、备注', content: d.notes });
