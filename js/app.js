...
(function () {
    ...
    const App = {
        ...
        init() {
            ...
            try { this.initCloudSync(); } catch (e) { console.error('initCloudSync:', e); }
            ...
            el.innerHTML = `
              <div class="hero">
                ...
                <div class="hero-content">
                  ...
                  <div class="cloud-status" id="cloud-status-bar">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 12l4-4"/><path d="M12 12V6"/></svg>
                    <span id="cloud-status-text">云同步未配置</span>
                    <button class="cloud-sync-btn" id="cloud-pull-btn" title="从云端拉取" style="display:none">↓ 拉取</button>
                    <button class="cloud-sync-btn" id="cloud-push-btn" title="上传到云端" style="display:none">↑ 上传</button>
                  </div>
                </div>
              </div>
        ...
        updateCloudStatus() {
            ...
            if (status.lastSyncTime) {
                ...
                const time = d.toLocaleString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
                const icon = status.lastSyncStatus === 'ok' ? '✓' : '✗';
                const color = status.lastSyncStatus === 'ok' ? 'var(--success, #34d399)' : 'var(--danger, #f87171)';
                textEl.innerHTML = `<span style="color:${color}">${icon}</span> 同步成功 ${time}`;
            } else {
                textEl.textContent = '☁ 云同步已启用';
            }
        },
              ...
              <div class="modal-overlay" id="ai-config-modal">
                <div class="modal">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="settings-divider"></div>
                    <h4 style="font-size:14px;font-weight:600;margin-bottom:12px;color:var(--text);">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-3px;margin-right:6px;"><path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 12l4-4"/><path d="M12 12V6"/></svg>Cloudflare Workers 云同步
                    </h4>
                    <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">通过 Cloudflare Workers KV 免费同步数据（手机和电脑互通）。需部署 Worker 后才能使用。</p>
                    <div class="form-row">
                      <div class="form-group flex-1"><label>API 地址 <span class="required">*</span></label><input type="text" id="lc-apiurl" placeholder="https://你的项目.workers.dev"></div>
                      <div class="form-group flex-1"><label>Token（可选）</label><input type="password" id="lc-token" placeholder="自定义密钥"></div>
                    </div>
                    <div class="form-row" style="align-items:center;gap:16px;">
                      <label class="checkbox-label" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;">
                        <input type="checkbox" id="lc-enabled" style="width:16px;height:16px;accent-color:var(--accent);"> 启用云同步
                      </label>
                    </div>
                  </div>
                  <div class="modal-footer">
                    ...
                    <button class="btn btn-info" id="lc-test-btn"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 12l4-4"/><path d="M12 12V6"/></svg> 测试连接</button>
        ...
        bindAIConfig() {
            ...
            btn.addEventListener('click', async () => {
                ...
                const lcfg = CloudSync.getConfig();
                try {
                    ...
                    document.getElementById('lc-apiurl').value = lcfg.apiUrl || '';
                    document.getElementById('lc-token').value = lcfg.token || '';
                    document.getElementById('lc-enabled').checked = lcfg.enabled || false;
            ...
            document.getElementById('lc-test-btn').addEventListener('click', () => this.testCloudSync());
            ...
            document.getElementById('cloud-pull-btn').addEventListener('click', async () => {
                try {
                    ...
                    this.updateCloudStatus();
                    this.updateStats();
                    BackupService.showToast('已从云端拉取 ' + count + ' 条数据', 'success');
                } catch (e) {
                    BackupService.showToast('拉取失败：' + e.message, 'error');
                }
            });
            document.getElementById('cloud-push-btn').addEventListener('click', async () => {
                try {
                    ...
                    this.updateCloudStatus();
                    BackupService.showToast('已上传到云端', 'success');
                } catch (e) {
                    BackupService.showToast('上传失败：' + e.message, 'error');
                }
            });
        ...
        async saveAIConfig() {
            ...
            CloudSync.saveConfig({
                apiUrl: document.getElementById('lc-apiurl').value.trim(),
                token: document.getElementById('lc-token').value.trim(),
                enabled: document.getElementById('lc-enabled').checked
            });
            this.updateCloudStatus();
            this.closeAIConfig();
            alert('配置已保存');
        ...
        async testCloudSync() {
            ...
            if (!cfg.apiUrl) { alert('请输入 API 地址'); return; }
            CloudSync.saveConfig(cfg);
            const btn = document.getElementById('lc-test-btn');
            btn.disabled = true;
            btn.innerHTML = '测试中…';
            try {
                ...
                btn.disabled = false;
                btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 12l4-4"/><path d="M12 12V6"/></svg> 测试连接';
            }
        },
