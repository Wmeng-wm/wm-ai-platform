...
const BackupService = {
    ...
    _getWebDAVUrl(path) {
        // 如果通过本地服务器访问（server.js），走代理解决 CORS
        if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
            return '/api/webdav?' + encodeURIComponent(path);
        }
        // 否则直连（仅限无 CORS 限制的环境）
    ...
    async syncToWebDAV() {
        ...
        const resp = await fetch(this._getWebDAVUrl(filename), {
            method: 'PUT',
            headers: headers,
            body: json
        });
    ...
    async testWebDAV() {
        ...
        const resp = await fetch(this._getWebDAVUrl(''), {
            method: 'PROPFIND',
