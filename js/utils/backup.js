...
const BackupService = {
    ...
    _isServerMode() {
        return Storage._useServer === true;
    },
    ...
    async getConfig() {
        ...
        if (this._isServerMode()) {
            try {
                const resp = await fetch('/api/webdav-config');
                if (resp.ok) {
        ...
        try {
            ...
            this._config = saved ? JSON.parse(saved) : {
                webdavUrl: '', webdavUser: '', webdavPass: '', autoSync: false, interval: 30
            };
        } catch {
            this._config = { webdavUrl: '', webdavUser: '', webdavPass: '', autoSync: false, interval: 30 };
        }
        return this._config;
    },
    ...
    async saveConfig(config) {
        ...
        if (this._isServerMode()) {
            try {
                ...
                console.warn('saveConfig to server failed:', e.message);
            }
        }
        ...
        if (this._isServerMode()) {
            try {
                ...
                if (resp.ok) {
                    ...
                    if (showToastOnSuccess !== false) this.showToast('备份已下载', 'success');
                    return true;
                }
    ...
    async syncToWebDAV() {
        ...
        // 服务器模式：由服务器直接上传到 WebDAV（无 CORS 问题）
        if (this._isServerMode()) {
            const resp = await fetch('/api/webdav/sync', { method: 'POST' });
            if (!resp.ok) {
                const err = await resp.json().catch(() => ({}));
                throw new Error(err.error || 'WebDAV 同步失败 (HTTP ' + resp.status + ')');
            }
        ...
        let url = isLocal
            ? '/api/webdav?' + encodeURIComponent(filename)
            : cfg.webdavUrl + (cfg.webdavUrl.endsWith('/') ? '' : '/') + filename;
        ...
        const resp = await fetch(url, { method: 'PUT', headers, body: json });
    ...
    async testWebDAV() {
        ...
        const isLocal = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
        const url = isLocal
            ? '/api/webdav?' + encodeURIComponent(cfg.webdavUrl.split('/').pop() || '')
            : cfg.webdavUrl + (cfg.webdavUrl.endsWith('/') ? '' : '/');

        const resp = await fetch(url, { method: 'PROPFIND', headers: { ...headers, Depth: '0' } });
    ...
    onDataChanged() {
        if (this._changeCount % 5 === 0) {
            ...
            if (cfg && cfg.autoSync && cfg.webdavUrl) {
                this.syncToWebDAV().catch(err => console.warn('Auto sync failed:', err.message));
            } else if (this._changeCount % 10 === 0) {
                this.showToast('已有 ' + this._changeCount + ' 条变更，建议备份数据', 'info');
            }
        }
    },
